...
def lineGraph():
    ...
    # Make a data frame
    df=pd.DataFrame({'Milliseconds': range(1,5), 'Web Scraping': webScrapingTimes, 'Calculating Primes': calculatePrimeTimes, 'Compression Times': compressionTimes})
    ...
    for column in df.drop('Milliseconds', axis=1):
        num+=1
        plt.plot(df['Milliseconds'], df[column], marker='', color=palette(num), linewidth=1, alpha=0.9, label=column)
